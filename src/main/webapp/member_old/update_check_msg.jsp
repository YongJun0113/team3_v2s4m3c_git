    ...
    <UL>
        ...
        <c:when test="${password_cnt == 1 }"> <!-- 패스워드 일치 -->
          <c:choose>
            <c:when test="${cnt == 1}"> <!-- 글 수정 성공 -->
              <LI class='li_none'>
                <span class='span_success'>패스워드가 일치합니다.</span>
              </LI>
            </c:when>
            <c:otherwise>    <!-- 글 수정 실패 -->
              <LI class='li_none'>
      ...
      <c:choose>
        <c:when test="${cnt == 1 && password_cnt == 1}">
          <LI class='li_none'>
            <button type='button' 
                        onclick="location.href='./update.do?memberno=${param.memberno}'"
                        class="btn btn-info">변경 하기</button>
            <button type='button' 
                        onclick="location.href='./list.do?memberno=${param.memberno}'"
                        class="btn btn-info">변경 취소</button>                        
          </LI>
        </c:when>
        <c:otherwise>
          <LI class='li_none'>
