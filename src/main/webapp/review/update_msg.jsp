        ...
        <c:when test="${passwd_cnt == 1 }"> <!-- 패스워드 일치 -->
          <c:choose>
            <c:when test="${cnt == 1}"> <!-- 글 수정 성공 -->
              <LI class='li_none'>
                <span class='span_success'>리뷰를 수정했습니다.</span>
              </LI>
            </c:when>
            <c:otherwise>    <!-- 글 수정 실패 -->
              <LI class='li_none'>
      ...
      <c:choose>
        <c:when test="${cnt == 1 && passwd_cnt == 1}">
          <LI class='li_none'>
            ...
            <button type='button' 
                        onclick="locations.href='./list.do?review_no=${param.review_no}'"
        ...
        <c:otherwise>
          <LI class='li_none'>
            ...
            <button type='button' 
                        onclick="location.href='./list.do?review_no=${param.review_no}'"
                        class="btn btn-info">목록</button>                        
          </LI>
        </c:otherwise> 
      </c:choose>
