...
import dev.mvc.ncate.NcateProcInter;
...
//  }
  ...
  public ModelAndView list_by_ncateno(int ncate_no) { 
    
    ModelAndView mav = new ModelAndView(); 
    mav.setViewName("/notice/list_by_ncateno");
    
    NcateVO ncateVO = this.ncateProc.read(ncate_no); 
    mav.addObject("ncateVO", ncateVO);
 
    List<NoticeVO> list = this.noticeProc.list_by_ncateno(ncate_no);
    mav.addObject("list", list);
    return mav; 
  ...
  public ModelAndView update_visible(NoticeVO noticeVO) {
    ...
    mav.setViewName("redirect:/notice/list_all.do"); // request 객체가 전달이 안됨. 
  ...
  /**
   * 목록 + 검색 지원
   * http://localhost:9090/resort/notice/list.do
   * http://localhost:9090/resort/notice/list.do?ncateno=1&title=스위스
   * @param ncate_no
   * @param title
   * @return
   */
...
//  public ModelAndView list_by_ncateno_search(
//      @RequestParam(value="ncate_no", defaultValue="1") int ncate_no,
//      @RequestParam(value="title", defaultValue="") String title
//      ) { 
//    
//    ModelAndView mav = new ModelAndView();
//    // /contents/list_by_cateno_table_img1_search.jsp
//    mav.setViewName("/notice/list_by_ncateno_search");   
//    
//    // 숫자와 문자열 타입을 저장해야함으로 Obejct 사용
//    HashMap<String, Object> map = new HashMap<String, Object>();
//    map.put("ncate_no", ncate_no); // #{cateno}
//    map.put("title", title);     // #{word}
//    
//    // 검색 목록
//    List<NoticeVO> list = noticeProc.list_by_ncateno_search(map);
//    mav.addObject("list", list);
//    
//    // 검색된 레코드 갯수
//    int search_cnt = noticeProc.search_cnt(map);
//    mav.addObject("search_cnt", search_cnt);
//
//    NcateVO ncateVO = ncateProc.read(ncate_no);
//    mav.addObject("ncateVO", ncateVO);
//    
//    return mav;
//  }    
  ...
  public ModelAndView list_by_cateno_search_paging(
    ...
    // 검색 목록
    List<NoticeVO> list = noticeProc.list_by_ncateno_search_paging(map);
    mav.addObject("list", list);
    
    // 검색된 레코드 갯수
    int search_count = noticeProc.search_cnt(map);
     ...
     * @return 페이징 생성 문자열
     */ 
    String paging = noticeProc.pagingBox("list.do", ncate_no, search_count, nowPage, title);
    mav.addObject("paging", paging);
  
    mav.addObject("nowPage", nowPage);

    // /contents/list_by_cateno_table_img1_search_paging.jsp
    mav.setViewName("/notice/list_by_ncateno_search_paging");   
